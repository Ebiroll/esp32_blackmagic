...
{
	struct stm_port_interface *port = stm->port;
	uint8_t byte;
	port_err_t p_err;
	time_t t0, t1;
...
stm32_t *stm32_init(struct stm_port_interface *port, const char init)
{
...
stm32_t		*stm		= NULL;
struct stm_port_interface *port = NULL;
char		init_flag	= 1;

// Set STM port struct
extern stm_port_interface_t *set_stm_port();



// This will scan the device that should be in boot mode (Press BOOT0 at starup)
void scan_uart_boot_mode()
 {
	...
	gdb_outf( "Version      : 0x%02x\n", stm->bl_version);
	if (port->flags & PORT_GVR_ETX) {
		gdb_outf( "Option 1     : 0x%02x\n", stm->option1);
		gdb_outf( "Option 2     : 0x%02x\n", stm->option2);
	}
	gdb_outf( "Device ID    : 0x%04x (%s)\n", stm->pid, stm->dev->name);
	gdb_outf( "- RAM        : Up to %dKiB  (%db reserved by bootloader)\n", (stm->dev->ram_end - 0x20000000) / 1024, stm->dev->ram_start - 0x20000000);
	gdb_outf( "- Flash      : Up to %dKiB (size first sector: %dx%d)\n", (stm->dev->fl_end - stm->dev->fl_start ) / 1024, stm->dev->fl_pps, stm->dev->fl_ps[0]);
	gdb_outf( "- Option RAM : %db\n", stm->dev->opt_end - stm->dev->opt_start + 1);
	gdb_outf( "- System RAM : %dKiB\n", (stm->dev->mem_end - stm->dev->mem_start) / 1024);
